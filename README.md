...
- [Generate Flow](src/Docs/Generate-Flow_Architecture.md) : Documents the end-to-end POST /v1/Architecture/generate flow, covering request validation, layer inference, dependency rules, code-generation steps, and response assembly.
- [Get PowerShell Helpers Flow](src/Docs/PowerShell-Helpers-Flow_Architecture.md) :  Documents the generation of a runtime-resolved, PowerShell script that exposes wrapper functions for all Architecture API endpoints to simplify and harden AI/human invocations while remaining a Host-layer presentation concern.
- [Get MCP Manifest Flow](src/Docs/MCP-Manifest-Flow_Architecture.md) :  High-level design and runtime flow for the GET /v1.0/Mcp/manifest endpoint, detailing components, URL resolution, reflection-based schema generation, and how architecture rules are exposed to AI agents.
...
- **Generate C# components** — scaffolds files (class, record, interface, struct) for the given component roles, with correct namespace, `using` statements, constructor injection stubs, and interface implementations; layer is inferred from the role when not specified, see also: [Generate-Flow_Architecture.md](src/Docs/Generate-Flow_Architecture.md)
...
- **MCP manifest** — exposes a machine-readable manifest (`/v1.0/Mcp/manifest`) that AI clients use to discover all available actions, see also: [MCP-Manifest-Flow_Architecture.md](src/Docs/MCP-Manifest-Flow_Architecture.md)
- **PowerShell helpers** — exposes ready-to-use PowerShell functions (`/v1.0/PowerShell/helpers`) that AI agents load before calling the API, see also: [PowerShell-Helpers-Flow_Architecture.md](src/Docs/PowerShell-Helpers-Flow_Architecture.md)
...
```bash
git clone https://antoinegeboers.visualstudio.com/AIResearch/_git/AIResearch.Architecture
...
Copy the **Architecture MCP Tool [architecture-mcp.prompt.md](src/Tool/architecture-mcp.prompt.md) Instructions** file into that folder.
...
Describe what should exist — not where it belongs. The MCP tool determines placement.

You can find an example [example.prompt.md](src/Tool/example.prompt.md) in the Tool folder.
